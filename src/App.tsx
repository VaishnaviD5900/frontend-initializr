...
import { Download, Code2, Layers, Palette, GitBranch, Database, CheckCircle2 } from 'lucide-react'
import type { ProjectConfig, Framework, Styling, Routing, StateManagement } from './types'
...
function DevIcon({ name, size = 28 }: { name: string; size?: number }) {
  return (
    <i
      className={`devicon-${name}-plain colored`}
      style={{ fontSize: size }}
    />
  )
}
...
const FRAMEWORKS: { value: Framework; label: string; icon: string; desc: string }[] = [
  { value: 'react',   label: 'React',   icon: 'react',   desc: 'v18 · Vite · JSX/TSX' },
  { value: 'vue',     label: 'Vue',     icon: 'vuejs',   desc: 'v3 · Vite · Composition API' },
...
function Section({ icon, title, children, step }: SectionProps) {
  return (
    <div className="mb-10">
    ...
    >
      ...
      <div className="px-6 py-5 border-b" style={{ borderColor: T.border }}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-medium tracking-wide" style={{ color: T.accent }}>
            Project Summary
          </span>
        </div>
        <p className="font-mono text-base font-semibold mt-1" style={{ color: '#f1f5f9' }}>
          {cfg.projectName}/
        </p>
      </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: T.bg, color: '#cbd5e1' }}>
      ...
      {/* Header */}
      <header
        className="relative border-b px-8 py-4 flex items-center justify-between"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-mono font-semibold text-sm" style={{ color: '#f1f5f9' }}>
              Frontend Initializr
            </div>
            <div className="text-xs" style={{ color: T.muted }}>
              Scaffold your frontend in seconds
            </div>
          </div>
        </div>
        <div
      ...
      <div className="relative max-w-6xl mx-auto px-8 py-12 grid grid-cols-3 gap-12">
        ...
        <div className="col-span-2">
          ...
          {/* Framework */}
          <Section step={1} icon={<Code2 size={13} />} title="Framework">
          ...
          {/* Styling */}
          <Section step={2} icon={<Palette size={13} />} title="Styling">
            <div className="grid grid-cols-3 gap-3">
          ...
          {/* Language */}
          <Section step={3} icon={<Code2 size={13} />} title="Language">
          ...
          {/* Routing */}
          <Section step={4} icon={<GitBranch size={13} />} title="Routing">
            <div className="grid grid-cols-3 gap-3">
          ...
          {/* State */}
          <Section step={5} icon={<Database size={13} />} title="State Management">
            <div className="grid grid-cols-3 gap-3">
        ...
        <div className="col-span-1">
          ...
          {done && (
            ...
            >
              ...
              <span>
                Done! Run{' '}
                <code className="font-mono text-xs">npm install && npm run dev</code>
              </span>
            </div>
          )}
        </div>
